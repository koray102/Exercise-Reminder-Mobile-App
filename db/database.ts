...
async function initDatabase(database: SQLite.SQLiteDatabase): Promise<void> {
  ...
  // Migration: add last_routine_completed_at column to categories
  try {
    await database.runAsync('ALTER TABLE categories ADD COLUMN last_routine_completed_at TEXT');
    console.log('[DB] Migration: added last_routine_completed_at column to categories');
  } catch (e: any) {
    if (!e.message?.includes('duplicate column')) {
